...
const warehouseModel = require("../Models/warehouseModels.js");
...
const createUser = async (req, res) => {
    ...
    try {
        const existUser = await warehouseModel.findOne({ username : username });
    if(existUser){
...
const getUser = async (req, res) => {
    try {
        const user = await warehouseModel.find();
...
const getSingleUser = async (req, res) => {
    ...
    try {
        const oneUser = await warehouseModel.findOne({_id : id})
...
const loginUser = async (req, res) => {
    const {username, password} = req.body;
    try {
        const existinguser = await warehouseModel.findOne({ username: username });
    if (!existinguser) {
...
const updateUser = async (req, res) => {
    ...
    try {
        bcrypt.hash(password, 10, async function (err, hash) {
            ...
            await warehouseModel.findByIdAndUpdate(id, newUser, {new : true});
...
const deleteUser = async (req, res) => {
    const id = req.params.id;
    try {
      const Users = await warehouseModel.findByIdAndRemove(id);
