...
const Csr = () => {
  ...
  const processhandleSubmit = async (e) => {
    ...
    e.preventDefault();
    await axios
      .put(`${API_URL}packages/${processId}`, process)
      .then((res) => {
  ...
  const csrhandleSubmit = async (e) => {
    ...
    e.preventDefault();
    await axios
      .put(`${API_URL}packages/${csrId}`, csrnotes)
      .then((res) => {
        setresponse(res.data.message);
        setcsrnote({
          csrnote: "",
        ...
        <div className={Style.csrContainer}>
          ...
          <div className={Style.csrFiler}>
            <div className={Style.crsFilterContainer}>
              ...
              />
              <input
                disabled={filterdate.length > 0 && true}
                type="number"
                name="filtertracking"
                id="filtertracking"
                  ...
                  slicePost
                    .filter((data) => {
                      ...
                      return filterdate.length > 0
                        ? dates
                        : filtertracking.length > 0
                        ? data.tracking
                            .toLowerCase()
                            .includes(filtertracking.toLowerCase())
                        : filterupc.length > 0
                        ? data.upc
                            .toLowerCase()
                      ...
                      return (
                        <tr key={item._id}>
                          ...
                          <td>
                            {item.processed}
                            {username && (
                              <span onClick={() => setprocessform(true)}>
                                <FaRegEdit
                                  className={Style.edit}
                                  onClick={() => editPackages(item._id)}
                                />
                              </span>
                            )}
                          </td>
          ...
          <form onSubmit={processhandleSubmit}>
            ...
            <span>
              ...
              <select
                name="processed"
                id="processed"
                onChange={(e) => processhandleChange(e)}
              >
          ...
          <form onSubmit={csrhandleSubmit}>
            ...
            <span>
              ...
              <textarea
                ...
                rows="10"
                value={csrnote}
                onChange={(e) => csrhandleChange(e)}
                placeholder="Ad your note"
              ></textarea>
            </span>
            <button type="submit">{loading ? "Updating..." : "Update"}</button>
          </form>
